...
public class CtplrIK : MonoBehaviour
{
    ...
    private void Init()
    {
        ...
        _boneLengths = new float[_chainLength];
        _completeLength = 0f;

        //rotations
        _initialDirection = new Vector3[_chainLength + 1];
        _initalRots = new Quaternion[_chainLength + 1];
    ...
    private void ResolveIK()
    {
        ...
        if (_boneLengths.Length != _chainLength)
            Init();
        ...
        else
        {
            ...
            for (int i = 0; i < _iterations; i++)
            {
                ...
                bool closeEnough = (_positions[_positions.Length - 1] - _target.position).sqrMagnitude < _delta * _delta;
                if (closeEnough)
                    break;
            }
        }
